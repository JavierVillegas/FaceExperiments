...
void testApp::draw(){
    ...
    Rewarp_corners[0] = cvPoint(1*Rx/4.0,1*Ry/4.0);
    Rewarp_corners[1] = cvPoint(3*Rx/4.0,1*Ry/4.0);
    Rewarp_corners[2] = cvPoint(3*Rx/4.0,3*Ry/4.0);
    Rewarp_corners[3] = cvPoint(1*Rx/4.0,3*Ry/4.0);
    ...
    ofxCvColorImage TempoRGB2;
    TempoRGB2.allocate(WarpDestiny.cols, WarpDestiny.rows);
    TempoRGB2 = WarpDestiny.data;
    TempoRGB2.draw(TempoRGB.width, 0);
    
    // no linear warping
    cv::Mat NonLinearDest;
    NonLinearDest.create( WarpDestiny.size(), WarpDestiny.type() );
    cv::remap( WarpDestiny, NonLinearDest, map_x, map_y, CV_INTER_LINEAR, cv::BORDER_CONSTANT, cv::Scalar(0,0, 0));
    
    ofxCvColorImage TempoRGB3;
    TempoRGB3.allocate(NonLinearDest.cols, NonLinearDest.rows);
    TempoRGB3 = NonLinearDest.data;
    TempoRGB3.draw(TempoRGB.width+TempoRGB2.width, 0);
